...
#' @examples
#'
stepwise_regression_beta <- function(tibble, dep_var, indep_oi, grps) {
  ...
  output <- tibble %>%
    ...
    mutate(dep_var = dep_var,
           ...
           full_sign = case_when(full_t_sign == TRUE & full_f_sign == TRUE ~ TRUE,
                                 TRUE ~ FALSE),
           forw_mod = map(data, ~ stepAIC(lm(as.formula(min), data = .), scope = max, direction = "forward", trace = FALSE)),
           ...
           forw_sign = case_when(forw_t_sign == TRUE & forw_f_sign == TRUE ~ TRUE,
                                 TRUE ~ FALSE),
           step_mod = map(data, ~ stepAIC(lm(as.formula(min), data = .), scope = max, direction = "both", trace = FALSE)),
